...
Each run is a single wake cycle:
  ...
  7. Deep-sleep for CYCLE_INTERVAL_MS

On wake from deep sleep, execution restarts here from the top.
RTC memory persists through deep sleep, preserving cycle position.
"""
...
# ---------------------------------------------------------------------------
# Cycle state helpers (persisted through deep sleep via flash file)
# ---------------------------------------------------------------------------
...
def _write_cycle_state(area_index: int, page_index: int) -> None:
    """Persist (area_index, page_index) to flash before deep sleep."""
...
if not area_ids:
    ...
    wifi.disconnect()
    machine.deepsleep(ERROR_RETRY_MS)  # type: ignore[attr-defined]  # no-return
...
logging.info(
    ...
    next_area_index, next_page_index, CYCLE_INTERVAL_MS // 1000,
)

machine.deepsleep(CYCLE_INTERVAL_MS)  # type: ignore[attr-defined]  # no-return
